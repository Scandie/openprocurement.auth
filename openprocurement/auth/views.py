...
@oauth_provider.route('/oauth/token/v2', methods=['GET', 'POST'])
@oauth.token_handler
def access_token_v2():
    return None


@oauth_provider.route('/oauth/authorize/v2', methods=['GET', 'POST'])
@oauth.authorize_handler
def authorize_v2(*args, **kwargs):
    ...
    if request.method == 'GET' and 'auto_allow' not in request.args:
        ...
        kwargs['user'] = user
        kwargs['redirect_uri'] = request.args['redirect_uri']
        kwargs['oauth_url'] = '/oauth/authorize/v2'
        response = make_response(render_template('authorize.html', **kwargs))
        response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0, max-age=0'
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '-1'
